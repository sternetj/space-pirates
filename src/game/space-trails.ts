...
export class SpaceTrails {
  ...
  public update() {
    this.container.children.forEach(this.updateTrail as any);
  }

  private updateTrail = (trail: PIXI.Sprite) => {
    trail.y += (trail as any).vy;
  ...
  private createTrail(offset = 0) {
    ...
    grd.addColorStop(
      1,
      `rgba(255, 255, 255, 0.${random.number({ min: 60, max: 99 })})`
    );
    ...
    (sprite as any).vy = random.number({ min: 5, max: 10 });
